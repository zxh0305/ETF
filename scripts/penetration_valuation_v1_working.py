...
logger = logging.getLogger("penetration_valuation_v1_working")
...
def load_sw_industry_data() -> Dict:
    ...
    # 转换为字典：行业代码 → {pe, pb, pe_percentile, pb_percentile}
    sw_dict = {}
...
def calculate_weighted_valuation(
    ...
    # 所有成分股都用同一个行业PE/PB（简化版）
    weighted_pe = sw_pe * (total_weight / 100)  # 权重是百分比
...
def main():
    """主函数"""
    logger.info("=" * 70)
    logger.info("穿透估值计算器 v1.0（完整可用版） - 基于持仓+申万行业加权")
    logger.info("=" * 70)
    ...
    output = {
        "meta": {
            ...
            "version": "v1.0",
            "description": "穿透估值计算（完整可用版：持仓+申万行业加权）",
